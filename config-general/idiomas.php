<?php

//ESPAÑOL

$l = 1;

/**
 * Propuesta #2 parea manejar los idiomas
*/
$language = [
    'social.amigos' => [
        'ES' => 'Amigos',  1 => 'Amigos',
        'EN' => 'Friends', 2 => 'Friends'
    ],
    'academico.clases' => [
        'ES' => 'Clases', 1 => 'Clases',
        'EN' => 'Lesson', 2 => 'Lesson'
    ]
];

$frases[1][$l] = 'Amigos';

$frases[2][$l] = 'Enviar mensaje';

$frases[3][$l] = 'Editar perfil';

$frases[4][$l] = 'Inicio';

$frases[5][$l] = 'Cursos';

$frases[6][$l] = 'Calificaciones';

$frases[7][$l] = 'Clases';

$frases[8][$l] = 'Buscar';

$frases[9][$l] = 'Resultados';

$frases[10][$l] = 'Mi perfil';

$frases[11][$l] = 'Mapa mundial';

$frases[12][$l] = 'Cargas académicas';

$frases[13][$l] = 'Administrar';

$frases[14][$l] = 'Configurar';

$frases[15][$l] = 'Salir';

$frases[16][$l] = 'Ayuda';

$frases[17][$l] = 'Configuración';

$frases[18][$l] = 'Idioma';

$frases[19][$l] = 'Seleccione una opción';

$frases[20][$l] = 'Español';

$frases[21][$l] = 'Inglés';

$frases[22][$l] = 'Tema';

$frases[23][$l] = 'Azul';

$frases[24][$l] = 'Naranja';

$frases[25][$l] = 'Verde';

$frases[26][$l] = 'Grado';

$frases[27][$l] = 'Periodo';

$frases[28][$l] = 'Docente';

$frases[29][$l] = 'Administrando';

$frases[30][$l] = 'Ausencias';

$frases[31][$l] = 'Importar notas';

$frases[32][$l] = 'Tipos de notas';

$frases[33][$l] = 'Redactar';

$frases[34][$l] = 'Entrada';

$frases[35][$l] = 'Favoritos';

$frases[36][$l] = 'Borrador';

$frases[37][$l] = 'Enviados';

$frases[38][$l] = 'Mensajes';

$frases[39][$l] = 'Ver';

$frases[40][$l] = 'Enviar';

$frases[41][$l] = 'Guardar';

$frases[42][$l] = 'Tareas';

$frases[43][$l] = 'Borrar';

$frases[44][$l] = 'Archivar';

$frases[45][$l] = 'Descartar';

$frases[46][$l] = 'para';

$frases[47][$l] = 'Asunto';

$frases[48][$l] = 'Mover';

$frases[49][$l] = 'Cod';

$frases[50][$l] = 'Descripci&oacute;n';

$frases[51][$l] = 'Fecha';

$frases[52][$l] = 'Valor';

$frases[53][$l] = 'Tipo';

$frases[54][$l] = 'Acciones';

$frases[55][$l] = 'Estudiantes';

$frases[56][$l] = 'Agregar';

$frases[57][$l] = 'Imprimir';

$frases[58][$l] = 'Guardar como PDF';

$frases[59][$l] = 'Exportar a excel';

$frases[60][$l] = 'Matr&iacute;cula';

$frases[61][$l] = 'Estudiante';

$frases[62][$l] = 'Definitiva';

$frases[63][$l] = 'Indicadores';

$frases[64][$l] = 'Tu tienes';

$frases[65][$l] = 'nuevos';

$frases[66][$l] = 'Todos';

$frases[67][$l] = 'Los cambios se han guardado correctamente';

$frases[68][$l] = 'Indicador';

$frases[69][$l] = 'Publicaciones';

$frases[70][$l] = 'leer m&aacute;s';

$frases[71][$l] = 'Acudidos';

$frases[72][$l] = 'Planes';

$frases[73][$l] = 'Asignaturas';

$frases[74][$l] = 'Compa&ntilde;eros';

$frases[75][$l] = 'Usuarios';

$frases[76][$l] = 'Docentes';

$frases[77][$l] = 'Acudientes';

$frases[78][$l] = 'Estudiantes';

$frases[79][$l] = 'Mis pedidos';

$frases[80][$l] = 'Buscador';

$frases[81][$l] = 'Resumen de notas';

$frases[82][$l] = 'Menu del sistema';

$frases[83][$l] = 'Administrador';

$frases[84][$l] = 'Resumen por periodos';

$frases[85][$l] = 'Mi matr&iacute;cula';

$frases[86][$l] = 'Donde estudiar mi carrera universitaria';

$frases[87][$l] = 'G. Administrativa';

$frases[88][$l] = 'G. Acad&eacute;mica';

$frases[89][$l] = 'G. Financiera';

$frases[90][$l] = 'G. Disciplinaria';

$frases[91][$l] = 'G. Social';

$frases[92][$l] = 'Perfiles';

$frases[93][$l] = '&Aacute;reas';

$frases[94][$l] = 'Materias';

$frases[95][$l] = 'Movimientos financieros';

$frases[96][$l] = 'Nuevo reporte';

$frases[97][$l] = 'Listado de reportes';

$frases[98][$l] = 'Muro Social';

$frases[99][$l] = 'Temática';

$frases[100][$l] = 'Escritorio';

$frases[101][$l] = 'Periodo actual';

$frases[102][$l] = 'Plataforma SINTIA';

$frases[103][$l] = 'Entrar';

$frases[104][$l] = 'Estado de cuenta';

$frases[105][$l] = 'Rep. Disciplinarios';

$frases[106][$l] = 'Periodos';

$frases[107][$l] = 'Total';

$frases[108][$l] = 'Nota';

$frases[109][$l] = 'Observaciones';

$frases[110][$l] = 'En linea';

$frases[111][$l] = 'Cronograma';

$frases[112][$l] = 'Tareas en casa';

$frases[113][$l] = 'Foros';

$frases[114][$l] = 'Evaluaciones';

$frases[115][$l] = 'Pagina informativa';

$frases[116][$l] = 'Asignatura';

$frases[117][$l] = 'MIN';

$frases[118][$l] = 'DEF';

$frases[119][$l] = 'Información';

$frases[120][$l] = 'Es la nota mínima que <mark>requiere obtener el estudiante en el último periodo</mark> para poder aporbar la asignatura. Si este espacio está vacío o con un guión (-) significa que <mark>desde ya ha aprobado la asignatura.</mark>';

$frases[121][$l] = 'Es la nota definitiva que ha obtenido el estudiante hasta el momento, teniendo en cuenta los periodos ya terminados.';

$frases[122][$l] = 'Normal';

$frases[123][$l] = 'Recuperación';

$frases[124][$l] = 'Nivelada';

$frases[125][$l] = 'Disponible';

$frases[126][$l] = 'Entrega';

$frases[127][$l] = 'Titulo';

$frases[128][$l] = 'Archivo';

$frases[129][$l] = 'Empezar ahora';

$frases[130][$l] = 'Disponible desde';

$frases[131][$l] = 'Disponible hasta';

$frases[132][$l] = 'Menú';

$frases[133][$l] = 'Publicidad';

$frases[134][$l] = 'Nueva publicación';

$frases[135][$l] = 'Mostrar mis publicaciones';

$frases[136][$l] = 'Ocultar mis publicaciones';

$frases[137][$l] = 'Eliminar mis publicaciones';

$frases[138][$l] = 'Género';

$frases[139][$l] = 'Preguntas';

$frases[140][$l] = 'Enviar evaluación';

$frases[141][$l] = 'Realizando';

$frases[142][$l] = 'Finalizado';

$frases[143][$l] = 'Consideraciones';

$frases[144][$l] = 'Cantidad de estudiantes que están presentando la evaluación en este momento, incluyendote';

$frases[145][$l] = 'Cantidad de estudiantes que ya enviaron esta evaluación';

$frases[146][$l] = 'Aquí nos hemos saltado una pregunta porque no tiene respuestas disponibles. Pero puedes continuar';

$frases[147][$l] = 'Si cambias de pestaña o sales de ella durante la evaluación, esta última <mark>quedará anulada.</mark>';

$frases[148][$l] = 'Verifica bien que hayas respondido todas las preguntas antes de enviar la evaluación. Las <mark>preguntas NO contestadas</mark> te restarán puntos.';

$frases[149][$l] = 'NO recargues o actualices esta página para que no se te cierre la evaluación o pierdas la información que llevas hasta entonces.';

$frases[150][$l] = 'Continuar';

$frases[151][$l] = 'Ingresa la contraseña de esta evaluación';

$frases[152][$l] = 'Contraseña';

$frases[153][$l] = 'El docente a cargo te debe proporcionar esta información';

$frases[154][$l] = 'Ver detalles';

$frases[155][$l] = 'La respuesta marcada fue la que tú elegiste. Al lado de cada respuesta, <mark>dentro de (paréntesis), aparecerá si es correcta o no.</mark> Por el color también la puedes distinguir. El verde para las correctas y el rojo para las incorrectas. Las respuesta sin marca significa que <mark>no las respondiste</mark> al momento de la evaluación.';

$frases[156][$l] = 'Correctas';

$frases[157][$l] = 'Incorrectas';

$frases[158][$l] = 'No contestadas';

$frases[159][$l] = 'Puedes ver el detalle también de estas evaluaciones que ya realizaste haciendo click sobre la evaluación.';

$frases[160][$l] = 'Cómo respondieron los estudiantes';

$frases[161][$l] = 'Te recomendamos estar pendiente del tiempo de la evaluación. Sin embargo el sistema te alertará cuando se te esté acabando el tiempo para que puedas enviar la evaluación con tiempo. <mark>Una vez finalizado el tiempo la evaluación se cerrará automáticamente.</mark>';

$frases[162][$l] = 'Detalle';

$frases[163][$l] = 'Reportes disciplinarios';

$frases[164][$l] = 'Curso';

$frases[165][$l] = 'Editar';

$frases[166][$l] = 'Opciones';

$frases[167][$l] = 'Importar información';

$frases[168][$l] = 'Publicación rápida';

$frases[169][$l] = '¿Qué quieres publicar?';

$frases[170][$l] = 'Publicar';

$frases[171][$l] = 'Cancelar';

$frases[172][$l] = 'Mostrar';

$frases[173][$l] = 'Ocultar';

$frases[174][$l] = 'Eliminar';

$frases[175][$l] = 'Servicios';

$frases[176][$l] = '<b>GENIAL:</b> En este espacio podrás encontrar servicios complementarios que requieras para ti y tus acudidos.';

$frases[177][$l] = 'Estas entidades son de total confianza. Están verificadas por nosotros. Te las recomendamos.';

$frases[178][$l] = 'Cuando contactes con alguna de estas entidades, diles que las encontraste en la plataforma SINTIA.';

$frases[179][$l] = 'Filtros de servicios';

$frases[180][$l] = 'Ver todo';

$frases[181][$l] = 'Email';

$frases[182][$l] = 'Teléfono';

$frases[183][$l] = 'Sitio web';

$frases[184][$l] = 'Regresar';

$frases[185][$l] = 'Verificada';

$frases[186][$l] = 'Usuario';

$frases[187][$l] = 'Nombre';

$frases[188][$l] = 'Celular';

$frases[189][$l] = 'Fecha de nacimiento';

$frases[190][$l] = 'Lugar de nacimiento';

$frases[191][$l] = 'Nivel académico';

$frases[192][$l] = 'Número de hijos';

$frases[193][$l] = 'Público';

$frases[194][$l] = 'Descargar';

$frases[195][$l] = 'Retrasos';

$frases[196][$l] = 'Detalles';

$frases[197][$l] = 'Enviar actividad';

$frases[198][$l] = 'Archivos adjuntos';

$frases[199][$l] = 'Desde esta opción puedes enviar la actividad pendiente. También puedes hacer un comentario, si lo deseas, sobre la actividad enviada para que el docente lo tenga presente.';

$frases[200][$l] = 'Archivos enviados';

$frases[201][$l] = 'Comentario';

$frases[202][$l] = 'Fecha de envío';

$frases[203][$l] = 'Ya has enviado un archivo relacionado a esta actividad. Si lo deseas reemplazar, puedes repetir el proceso inicial.';

$frases[204][$l] = 'Haz un comentario';

$frases[205][$l] = 'Filtros';

$frases[206][$l] = 'Puedes usar los filtros para cambiar rápidamente de asignatura o de periodo.';

$frases[207][$l] = 'Información actual';

$frases[208][$l] = 'Ten en cuenta que la información que agregues, modifiques o elimines se hará con estos datos.';

$frases[209][$l] = 'Matrículas';

$frases[210][$l] = 'G. Mercadeo';

$frases[211][$l] = 'Imagen';

$frases[212][$l] = 'Agregar publicación';

$frases[213][$l] = 'URL imagen';

$frases[214][$l] = 'URL Video YouTube';

$frases[215][$l] = 'Cumpleaños hoy';

$frases[216][$l] = 'Carpetas';

$frases[217][$l] = 'Editar publicación';

$frases[218][$l] = 'Notificaciones';

$frases[219][$l] = 'Foto';

$frases[220][$l] = 'Monitoreo';

$frases[221][$l] = 'Formatos';

$frases[222][$l] = 'Categorías';

$frases[223][$l] = 'Galería';

$frases[224][$l] = 'Categoría específica';

$frases[225][$l] = 'Nuevo monitoreo';

$frases[226][$l] = 'Evaluado';

$frases[227][$l] = 'Compartir';

$frases[228][$l] = 'Palabras claves';

$frases[229][$l] = 'Carpeta padre';

$frases[230][$l] = 'Quitar filtros';

$frases[231][$l] = 'Agregar nuevo';

$frases[232][$l] = 'Mi unidad';

$frases[233][$l] = 'Compartidas conmigo';

$frases[234][$l] = 'Comportamiento';

$frases[235][$l] = 'Búsqueda';

$frases[236][$l] = 'Felicitar';

$frases[237][$l] = 'Página';

$frases[238][$l] = 'ES MEJOR ESTAR SEGURO';

$frases[239][$l] = 'Si tienes dudas sobre alguna opción de esta pantalla, consulta el';

$frases[240][$l] = 'MANUAL DE AYUDA AQUÍ';

$frases[241][$l] = 'DNI';

$frases[242][$l] = 'Definitivas actuales';

$frases[243][$l] = 'Resumen de notas';

$frases[244][$l] = 'Recursos';

$frases[245][$l] = 'Calendario';

$frases[246][$l] = 'Estado';

$frases[247][$l] = 'Promedios estudiantiles';

$frases[248][$l] = 'Faltas';

$frases[249][$l] = 'Sacar reportes';

$frases[250][$l] = 'Grupo';

$frases[251][$l] = 'Temática';

$frases[252][$l] = 'Notas por indicador';

$frases[253][$l] = 'Cambiar clave';

$frases[254][$l] = 'Grupos';

$frases[255][$l] = 'Guia para empezar';

$frases[256][$l] = 'Perfil';

$frases[257][$l] = 'Soporte';

$frases[258][$l] = 'Mis páginas recientes';

$frases[259][$l] = 'Mis páginas más visitadas';

$frases[260][$l] = 'Buscar Información';

$frases[261][$l] = 'Inglés';

$frases[262][$l] = 'Español';

$frases[263][$l] = 'Crea una publicación más completa';

$frases[264][$l] = 'Aspectos';

$frases[265][$l] = 'Informe parcial';

$frases[266][$l] = 'Ficha estudiantil';

$frases[267][$l] = 'Descargar Boletín';

$frases[268][$l] = 'Bloqueado';

$frases[269][$l] = 'Solicitar desbloqueo';

$frases[270][$l] = '¿Desea reservar el cupo?';

$frases[271][$l] = 'Enviar solicitud';

$frases[272][$l] = 'Enviar respuesta';

$frases[273][$l] = 'Si cree que el estudiante está bloqueado por error, entonces envíe la solicitud de desbloqueo a los directivos de la Institución.';

$frases[274][$l] = 'Si desea puede escribir aquí un comentario';

$frases[275][$l] = 'SI';

$frases[276][$l] = 'NO';

$frases[277][$l] = 'Reserva de cupos';

$frases[278][$l] = '¿Desea reservar cupo del estudiante para el siguiente año escolar?';

$frases[279][$l] = 'Nos gustaría saber el motivo por el cual no desea reservar el cupo para este estudiante.';

$frases[280][$l] = 'Escriba el motivo';

$frases[281][$l] = 'Aspectos académicos';

$frases[282][$l] = 'Aspectos convivenciales';

$frases[283][$l] = 'Información de consulta';

$frases[284][$l] = 'Es el periodo actual que se está trabajando en la asignatura.';

$frases[285][$l] = 'Más opciones';

$frases[286][$l] = 'Firmar';

$frases[287][$l] = 'Firma digital';

$frases[288][$l] = 'Al hacer click en el botón <b>Enviar firma y comentario</b>, estás firmando y aceptando el reporte realizado a su acudido. Los comentarios colocados, llegarán a la persona encargada para ser tenidos en cuenta.';

$frases[289][$l] = 'Enviar firma y comentario';

$frases[290][$l] = 'Si desea puede escribir aquí un comentario';

$frases[291][$l] = 'Editar información';

$frases[292][$l] = 'Aspectos Estudiantiles';

$frases[293][$l] = 'Edad';

$frases[294][$l] = 'Tipo de';

$frases[295][$l] = 'Número de hermanos';

$frases[296][$l] = 'Lugar que ocupa entre ellos';

$frases[297][$l] = 'Direccion';

$frases[298][$l] = 'Barrio';

$frases[299][$l] = 'Parentezco';

$frases[300][$l] = 'Nombre papá';

$frases[301][$l] = 'Nombre mamá';

$frases[302][$l] = 'Descripción de la situación';

$frases[303][$l] = 'Aspectos positivos';

$frases[304][$l] = 'Aspectos a mejorar';

$frases[305][$l] = 'Tratamiento';

$frases[306][$l] = 'Observador del alumno';

$frases[307][$l] = 'Nombre del Usuario';

$frases[308][$l] = 'Promovido';

$frases[309][$l] = 'Tienes las cuentas al día. Excelente!';

$frases[310][$l] = 'Tienes saldo a favor.';

$frases[311][$l] = 'Tienes un saldo pendiente por pagar. Trata de ponerte al día lo antes posible.';

$frases[312][$l] = 'Resumen';

$frases[313][$l] = 'Cobros';

$frases[314][$l] = 'Pagos';

$frases[315][$l] = 'Saldo';

$frases[316][$l] = 'Recuerde que su código para hacer el pago es el siguiente';

$frases[317][$l] = 'Link de pago';

$frases[318][$l] = 'Nombre de la carpeta';

$frases[319][$l] = 'Primer apellido';

$frases[320][$l] = 'Segundo apellido';

$frases[321][$l] = 'Cod. Tesorería';

$frases[322][$l] = 'La dirección se debe colocar con abreviaciones como (CL, DG, AV, CR) y no puede contener ningún tipo de carácter como (-,#,.).';

$frases[323][$l] = 'Estrato';

$frases[324][$l] = 'Sistema';

$frases[325][$l] = 'Si necesita hacer un cambio de acudiente debe comunicarse vía WhatsApp, al número 318 347 9394.';

$frases[326][$l] = 'Documento';

$frases[327][$l] = 'Ocupación';

$frases[328][$l] = 'Haz click sobre la imagen o sobre el nombre del curso al que deseas entrar.';

$frases[329][$l] = 'No tienes cargas asignadas aún.';

$frases[330][$l] = 'Proceso de matrícula';

$frases[331][$l] = 'Actualizar datos.';

$frases[332][$l] = 'Pago matrícula.';

$frases[333][$l] = 'Contrato.';

$frases[334][$l] = 'Pagaré.';

$frases[335][$l] = 'Compromiso académico.';

$frases[336][$l] = 'Compromiso de convivencia.';

$frases[337][$l] = 'Manual de convivencia.';

$frases[338][$l] = 'Contrato mayores de 14 años.';

$frases[339][$l] = 'Firma matrícula.';

$frases[340][$l] = 'El valor a pagar es';

$frases[341][$l] = 'Para realizar el proceso de pago tiene dos opciones, escoja alguna de las dos';

$frases[342][$l] = 'Ingrese al link y con el código de estudiante pague por pse';

$frases[343][$l] = 'O pagar en la cuenta corriente de Bancolombia';

$frases[344][$l] = 'Adjuntar soporte';

$frases[345][$l] = 'Comprobante de pago';

$frases[346][$l] = 'Nota: Una vez realice el pago, debe adjuntar el soporte para verificación con tesorería.';

$frases[347][$l] = 'SI tiene algún problema con el pago por favor comunicarse a tesorería.';

$frases[348][$l] = 'TABLA DE COSTOS';

$frases[349][$l] = 'IR A PAGO POR PSE';

$frases[350][$l] = 'Enviar comprobante';

$frases[351][$l] = 'CONTRATO DE PRESTACIÓN DE SERVICIOS EDUCATIVOS';

$frases[352][$l] = 'En este espacio puede adjuntar la firma digital, si no tiene firma digital puede realizar la firma en un papel blanco sin líneas, ni cuadriculas; tomarle una foto con buena iluminación y subirla a este medio.';

$frases[353][$l] = 'Adjuntar firma.';

$frases[354][$l] = 'NOTA: Una vez la secretaria académica verifique y apruebe la matrícula le aparecerá el comprobante final.';

$frases[355][$l] = 'DESCARGAR COMPROBANTE';

$frases[356][$l] = 'Menú principal';

$frases[357][$l] = 'PRIMEROS PASOS';

$frases[358][$l] = 'Primeramente vamos a las asignaturas para seleccionar la que necesitamos.';

$frases[359][$l] = 'Exploremos las opciones del menú G. Académica.';

$frases[360][$l] = 'Veamos otras opciones adicionales.';

$frases[361][$l] = 'Video de Guia';

$frases[362][$l] = 'Todas las opciones más importantes están en la opción acudidos del menú. Te recomendamos ver el video de esta pagina para que tengas claridad sobre el manejo de la plataforma.';

$frases[367][$l] = 'Cancelar servicio';

$frases[368][$l] = 'Motivo de Cancelacion';

$frases[369][$l] = 'Crear';

$frases[370][$l] = 'Páginas Disponibles';

$frases[371][$l] = 'Páginas';

$frases[372][$l] = 'Institución';

$frases[373][$l] = 'Año';

$frases[374][$l] = 'Unidades Tematicas';

$frases[375][$l] = 'Editar';

$frases[376][$l] = 'Escoge la carga y el periodo desde donde quieres importar la información.';

$frases[377][$l] = 'Ahora puedes especificar la información que quieres importar.';

$frases[378][$l] = 'Al importar las calificaciones también se importarán los indicadores automáticamente.';

$frases[379][$l] = 'Verifica que hayas seleccionado todo correctamente para esta importación. Una vez hecha no hay vuelta atrás.';

$frases[380][$l] = 'Factura de Venta';

$frases[381][$l] = 'Precio';

$frases[382][$l] = 'Iva';

$frases[383][$l] = 'Responsable';

$frases[384][$l] = 'Ambiente';


$frases[385][$l] = 'Informes';
$frases[386][$l] = 'Busqueda';
$frases[387][$l] = 'Más acciones';
$frases[388][$l] = 'Cuestionarios';
$frases[389][$l] = 'Último ingreso';
$frases[390][$l] = 'Inscripciones';
$frases[391][$l] = 'Reserva de cupos';
$frases[392][$l] = 'Listado de inscripciones';
$frases[393][$l] = 'Ver cuestionarios';
$frases[394][$l] = 'Solicitud desbloqueo';
$frases[395][$l] = 'Del sistema';
$frases[396][$l] = 'De la institución';
$frases[397][$l] = 'Crear nueva compañía';
$frases[398][$l] = 'Log de errores';
$frases[399][$l] = 'Compañías';
$frases[400][$l] = 'Historial de acciones';
$frases[401][$l] = 'Solicitudes de cancelacion';
$frases[402][$l] = 'Módulos';
$frases[403][$l] = 'Paginas';
$frases[404][$l] = 'Opciones generales';
$frases[405][$l] = 'Claves restauradas';
$frases[406][$l] = 'Contratos';
$frases[407][$l] = 'Datos de contacto';
$frases[408][$l] = 'Categorías  productos';
$frases[409][$l] = 'Categorías servicios';
$frases[410][$l] = 'Productos';
$frases[411][$l] = 'Empresas';
$frases[412][$l] = 'Filtrar por cursos';
$frases[413][$l] = 'Abonos';
$frases[414][$l] = 'Método de Pago';

$frases[415][$l] = 'Facturas Recurrentes';
$frases[416][$l] = 'Notas Adicionales';
$frases[417][$l] = 'Cobrado';
$frases[418][$l] = 'Por cobrar';
$frases[419][$l] = 'Guardar cambios';
$frases[420][$l] = 'Obligatoria';
$frases[421][$l] = 'TEXTO';
$frases[422][$l] = 'MÚLTIPLE';
$frases[423][$l] = 'ÚNICA';
$frases[424][$l] = 'Cliente';
$frases[425][$l] = 'Impuesto';
$frases[426][$l] = 'Tarifa';
$frases[427][$l] = 'Reportes Gráficos';
$frases[428][$l] = 'Cursos Adicionales';

//ENGLISH

$l = 2;

$frases[1][$l] = 'Friends';

$frases[2][$l] = 'Send message';

$frases[3][$l] = 'Edit profile';

$frases[4][$l] = 'Home';

$frases[5][$l] = 'Courses';

$frases[6][$l] = 'Score';

$frases[7][$l] = 'Lesson';

$frases[8][$l] = 'Search';

$frases[9][$l] = 'Results';

$frases[10][$l] = 'My profile';

$frases[11][$l] = 'Map World';

$frases[12][$l] = 'Academic loads';

$frases[13][$l] = 'Manage';

$frases[14][$l] = 'Settings';

$frases[15][$l] = 'Logout';

$frases[16][$l] = 'Help';

$frases[17][$l] = 'Configuration';

$frases[18][$l] = 'Language';

$frases[19][$l] = 'Select an option';

$frases[20][$l] = 'Spanish';

$frases[21][$l] = 'English';

$frases[22][$l] = 'Theme';

$frases[23][$l] = 'Blue';

$frases[24][$l] = 'Orange';

$frases[25][$l] = 'Green';

$frases[26][$l] = 'Grade';

$frases[27][$l] = 'Period';

$frases[28][$l] = 'Teacher';

$frases[29][$l] = 'Administering';

$frases[30][$l] = 'Absences';

$frases[31][$l] = 'Import notes';

$frases[32][$l] = 'Types of notes';

$frases[33][$l] = 'Compose';

$frases[34][$l] = 'Inbox';

$frases[35][$l] = 'Starred';

$frases[36][$l] = 'Drafts';

$frases[37][$l] = 'Sent';

$frases[38][$l] = 'Message';

$frases[39][$l] = 'View';

$frases[40][$l] = 'Send';

$frases[41][$l] = 'Save';

$frases[42][$l] = 'Task';

$frases[43][$l] = 'Delete';

$frases[44][$l] = 'Archive';

$frases[45][$l] = 'Discard';

$frases[46][$l] = 'To';

$frases[47][$l] = 'Subject';

$frases[48][$l] = 'Move';

$frases[49][$l] = 'ID';

$frases[50][$l] = 'Description';

$frases[51][$l] = 'Date';

$frases[52][$l] = 'Value';

$frases[53][$l] = 'Type';

$frases[54][$l] = 'Actions';

$frases[55][$l] = 'Students';

$frases[56][$l] = 'Add';

$frases[57][$l] = 'Print';

$frases[58][$l] = 'Save as PDF';

$frases[59][$l] = 'Export to excel';

$frases[60][$l] = 'Enrollment';

$frases[61][$l] = 'Student';

$frases[62][$l] = 'Definitive';

$frases[63][$l] = 'Indicators';

$frases[64][$l] = 'You have';

$frases[65][$l] = 'news';

$frases[66][$l] = 'All';

$frases[67][$l] = 'The changes have been successfully saved';

$frases[68][$l] = 'Indicator';

$frases[69][$l] = 'Publications';

$frases[70][$l] = 'Read more';

$frases[71][$l] = 'My Students';

$frases[72][$l] = 'Plans';

$frases[73][$l] = 'Subjects';

$frases[74][$l] = 'Partners';

$frases[75][$l] = 'Users';

$frases[76][$l] = 'Teachers';

$frases[77][$l] = 'Attendants';

$frases[78][$l] = 'Students';

$frases[79][$l] = 'My orders';

$frases[80][$l] = 'Search';

$frases[81][$l] = 'Summary by score';

$frases[82][$l] = 'System menu';

$frases[83][$l] = 'Administrator';

$frases[84][$l] = 'Summary by periods';

$frases[85][$l] = 'My enrollment';

$frases[86][$l] = 'Where study my carrera universitaria';

$frases[87][$l] = 'Administrative M.';

$frases[88][$l] = 'Academic M.';

$frases[89][$l] = 'Financial M.';

$frases[90][$l] = 'Disciplinary M.';

$frases[91][$l] = 'Social M.';

$frases[92][$l] = 'Profiles';

$frases[93][$l] = 'Areas';

$frases[94][$l] = 'Subjects';

$frases[95][$l] = 'Financial movements';

$frases[96][$l] = 'New report';

$frases[97][$l] = 'List of reports';

$frases[98][$l] = 'Social Wall';

$frases[99][$l] = 'Thematic';

$frases[100][$l] = 'Desktop';

$frases[101][$l] = 'Current period';

$frases[102][$l] = 'Plataforma SINTIA';

$frases[103][$l] = 'Enter';

$frases[104][$l] = 'Account status';

$frases[105][$l] = 'Disciplinary Rep.';

$frases[106][$l] = 'Periods';

$frases[107][$l] = 'Total';

$frases[108][$l] = 'Score';

$frases[109][$l] = 'Observations';

$frases[110][$l] = 'On line';

$frases[111][$l] = 'Schedule';

$frases[112][$l] = 'Activities';

$frases[113][$l] = 'Forums';

$frases[114][$l] = 'Evaluations';

$frases[115][$l] = 'Information page';

$frases[116][$l] = 'Subject';

$frases[117][$l] = 'MIN';

$frases[118][$l] = 'DEF';

$frases[119][$l] = 'Information';

$frases[120][$l] = 'It is the minimum mark that <mark> you need to obtain in the last period </mark> to be able to provide the subject. If this space is empty or with a hyphen (-) it means that <mark> you have already approved the subject.</mark>';

$frases[121][$l] = 'It is the final note you have obtained so far, taking into account the periods already completed.';

$frases[122][$l] = 'Normal';

$frases[123][$l] = 'Recovery';

$frases[124][$l] = 'Leveled out';

$frases[125][$l] = 'Available';

$frases[126][$l] = 'Delivery';

$frases[127][$l] = 'Title';

$frases[128][$l] = 'File';

$frases[129][$l] = 'Start now';

$frases[130][$l] = 'Available since';

$frases[131][$l] = 'Available until';

$frases[132][$l] = 'Menu';

$frases[133][$l] = 'Publicity';

$frases[134][$l] = 'New publication';

$frases[135][$l] = 'Show my publications';

$frases[136][$l] = 'Hide my publications';

$frases[137][$l] = 'Delete my publications';

$frases[138][$l] = 'Gender';

$frases[139][$l] = 'Questions';

$frases[140][$l] = 'Send evaluation';

$frases[141][$l] = 'Doing';

$frases[142][$l] = 'Finished';

$frases[143][$l] = 'Considerations';

$frases[144][$l] = 'Cantidad de estudiantes que están presentando la evaluación en este momento, incluyendote.';

$frases[145][$l] = 'Cantidad de estudiantes que ya enviaron esta evaluación.';

$frases[146][$l] = 'Aquí nos hemos saltado una pregunta porque no tiene respuestas disponibles. Pero puedes continuar';

$frases[147][$l] = 'Si cambias de pestaña o sales de ella durante la evaluación, esta última <mark>quedará anulada.</mark>';

$frases[148][$l] = 'Verifica bien que hayas respondido todas las preguntas antes de enviar la evaluación. Las <mark>preguntas NO contestadas</mark> te restarán puntos.';

$frases[149][$l] = 'NO recargues o actualices esta página para que no se te cierre la evaluación o pierdas la información que llevas hasta entonces.';

$frases[150][$l] = 'Continue';

$frases[151][$l] = 'Enter password to this evaluation';

$frases[152][$l] = 'Password';

$frases[153][$l] = 'The current teacher should give you this information';

$frases[154][$l] = 'View details';

$frases[155][$l] = 'The answer marked was the one you chose. Next to each answer, <mark> inside (parentheses), it will appear if it is correct or not. </mark> By the color you can also distinguish it. The green for the correct ones and the red one for the incorrect ones. Unbranded responses mean that <mark> you did not answer them </mark> at the time of the evaluation.';

$frases[156][$l] = 'Correct';

$frases[157][$l] = 'Incorrect';


$frases[158][$l] = 'Not answered';

$frases[159][$l] = 'You can see the detail also of these evaluations that you already made by clicking on the evaluation.';

$frases[160][$l] = 'How the students responded';

$frases[161][$l] = 'We recommend you to be aware of the time of the evaluation. However, the system will alert you when you are running out of time so you can send the evaluation in time. <mark> Once the time has finished the evaluation will close automatically. </ mark>';

$frases[162][$l] = 'Detail';

$frases[163][$l] = 'Disciplinary reports';

$frases[164][$l] = 'Course';

$frases[165][$l] = 'Edit';

$frases[166][$l] = 'Options';

$frases[167][$l] = 'Import information';

$frases[168][$l] = 'Quick publication';

$frases[169][$l] = 'What do you want to publish?';

$frases[170][$l] = 'Post';

$frases[171][$l] = 'Cancel';

$frases[172][$l] = 'Show';

$frases[173][$l] = 'Hide';

$frases[174][$l] = 'Delete';

$frases[175][$l] = 'Services';

$frases[176][$l] = '<b>GREAT: </b> In this space you will find complementary services that you require for you and your visitors.';

$frases[177][$l] = 'These entities are completely trustworthy. They are verified by us. We recommend them.';

$frases[178][$l] = 'When you contact any of these entities, tell them you found them on the SINTIA platform.';

$frases[179][$l] = 'Service filters';

$frases[180][$l] = 'View all';

$frases[181][$l] = 'Email';

$frases[182][$l] = 'Telephone';

$frases[183][$l] = 'Web site';

$frases[184][$l] = 'Return';

$frases[185][$l] = 'Verified';

$frases[186][$l] = 'User';

$frases[187][$l] = 'Name';

$frases[188][$l] = 'Cell phone';

$frases[189][$l] = 'Birthdate';

$frases[190][$l] = 'Place of birth';

$frases[191][$l] = 'Academic level';

$frases[192][$l] = 'Number of children';

$frases[193][$l] = 'Public';

$frases[194][$l] = 'Download';

$frases[195][$l] = 'Delays';

$frases[196][$l] = 'Details';

$frases[197][$l] = 'Send activity';

$frases[198][$l] = 'Attached files';

$frases[199][$l] = 'From this option you can send the pending activity. You can also make a comment, if you wish, about the activity sent so that the teacher has it in mind.';

$frases[200][$l] = 'Archives sended';

$frases[201][$l] = 'Commentary';

$frases[202][$l] = 'Shipping date';

$frases[203][$l] = 'You have already sent a file related to this activity. If you want to replace it, you can repeat the initial process.';

$frases[204][$l] = 'Make a comment';

$frases[205][$l] = 'Filters';

$frases[206][$l] = 'You can use the filters to quickly change subject or period.';

$frases[207][$l] = 'Current information';

$frases[208][$l] = 'Keep in mind that the information you add, modify or delete will be made with this data.';

$frases[209][$l] = 'Enrollment';

$frases[210][$l] = 'Marketing M.';

$frases[211][$l] = 'Image';

$frases[212][$l] = 'Add post';

$frases[213][$l] = 'URL image';

$frases[214][$l] = 'URL Video YouTube';

$frases[215][$l] = 'Today birthday';

$frases[216][$l] = 'Folders';

$frases[217][$l] = 'Edit post';

$frases[218][$l] = 'Notifications';

$frases[219][$l] = 'Picture';

$frases[220][$l] = 'Monitoring';

$frases[221][$l] = 'Formats';

$frases[222][$l] = 'Categories';

$frases[223][$l] = 'Gallery';

$frases[224][$l] = 'Specific category';

$frases[225][$l] = 'New monitoring';

$frases[226][$l] = 'Evaluated';

$frases[227][$l] = 'Share';

$frases[228][$l] = 'Keywrods';

$frases[229][$l] = 'Parent folder';

$frases[230][$l] = 'Remove filters';

$frases[231][$l] = 'Add new';

$frases[232][$l] = 'My unity';

$frases[233][$l] = 'Shared with me';

$frases[234][$l] = 'Behavior';

$frases[235][$l] = 'Search';

$frases[236][$l] = 'Congratulate';

$frases[237][$l] = 'Page';

$frases[238][$l] = 'IT IS BETTER TO BE SAFE';

$frases[239][$l] = 'If you have doubts about any option on this screen, consult the';

$frases[240][$l] = 'HELP MANUAL HERE';

$frases[241][$l] = 'DNI';

$frases[242][$l] = 'Current scores';

$frases[243][$l] = 'Summary of scores';

$frases[244][$l] = 'Resourses';

$frases[245][$l] = 'Calendar';

$frases[246][$l] = 'Student averages';

$frases[247][$l] = 'Promedios estudiantiles';

$frases[248][$l] = 'Faults';

$frases[249][$l] = 'Take out reports';

$frases[250][$l] = 'Group';

$frases[251][$l] = 'Thematic';

$frases[252][$l] = 'Ratings by indicator';

$frases[253][$l] = 'Change password';

$frases[254][$l] = 'Groups';

$frases[255][$l] = 'Getting Started Guide';

$frases[256][$l] = 'Profile';

$frases[257][$l] = 'Support';

$frases[258][$l] = 'My recent pages';

$frases[259][$l] = 'My most visited pages';

$frases[260][$l] = 'search information';

$frases[261][$l] = 'English';

$frases[262][$l] = 'Spanish';

$frases[263][$l] = 'Create a more complete publication';

$frases[264][$l] = 'Aspects';

$frases[265][$l] = 'Partial report';

$frases[266][$l] = 'Student file';

$frases[267][$l] = 'Download Newsletter';

$frases[268][$l] = 'Blocked';

$frases[269][$l] = 'Request Unlock';

$frases[270][$l] = 'Do you want to reserve your quota?';

$frases[271][$l] = 'Send request';

$frases[272][$l] = 'Send reply';

$frases[273][$l] = 'If you believe that the student is blocked by mistake, then send the unblocking request to the Institutions management.';

$frases[274][$l] = 'If you wish, you can write a comment here';

$frases[275][$l] = 'YES';

$frases[276][$l] = 'NOT';

$frases[277][$l] = 'Quota reservation';

$frases[278][$l] = 'Would you like to reserve a students place for the next school year?';

$frases[279][$l] = 'We would like to know why you do not wish to reserve a place for this student.';

$frases[280][$l] = 'Write the reason';

$frases[281][$l] = 'Academic aspects';

$frases[282][$l] = 'Coexistence aspects';

$frases[283][$l] = 'Consultation information';

$frases[284][$l] = 'This is the current period being worked on in the subject.';

$frases[285][$l] = 'More options';

$frases[286][$l] = 'Sign';

$frases[287][$l] = 'Digital signature';

$frases[288][$l] = 'By clicking on the <b>Send signature and comment</b> button, you are signing and accepting the report made to your client. The comments posted will be forwarded to the person in charge to be taken into account.';

$frases[289][$l] = 'Send signature and comment';

$frases[290][$l] = 'If you wish, you can write a comment here';

$frases[291][$l] = 'Edit information';

$frases[292][$l] = 'Student Aspects';

$frases[293][$l] = 'Age';

$frases[294][$l] = 'type of';

$frases[295][$l] = 'Number of siblings';

$frases[296][$l] = 'Place it occupies among them';

$frases[297][$l] = 'address';

$frases[298][$l] = 'neighborhood';

$frases[299][$l] = 'relationship';

$frases[300][$l] = 'Fathers name';

$frases[301][$l] = 'Mothers name';

$frases[302][$l] = 'Description of the situation';

$frases[303][$l] = 'Positive aspects';

$frases[304][$l] = 'Aspects to improve';

$frases[305][$l] = 'Treatment';

$frases[306][$l] = 'Student observer';

$frases[307][$l] = 'Users name';

$frases[308][$l] = 'Promoted';

$frases[309][$l] = 'Your accounts are up to date. Excellent!';

$frases[310][$l] = 'You have a credit balance.';

$frases[311][$l] = 'You have a balance due. Try to catch up as soon as possible.';

$frases[312][$l] = 'Summary';

$frases[313][$l] = 'Collections';

$frases[314][$l] = 'Payments';

$frases[315][$l] = 'Balance';

$frases[316][$l] = 'Remember that your payment code is as follows';

$frases[317][$l] = 'Payment link';

$frases[318][$l] = 'Folder name';

$frases[319][$l] = 'First surname';

$frases[320][$l] = 'second last name';

$frases[321][$l] = 'Cod. treasury';

$frases[322][$l] = 'The address must be entered with abbreviations such as (CL, DG, AV, CR) and cannot contain any type of character such as (-,#,.).';

$frases[323][$l] = 'Stratum';

$frases[324][$l] = 'ID System';

$frases[325][$l] = 'If you need to make a change of guardian, please contact us via WhatsApp at 318 347 9394.';

$frases[326][$l] = 'Document';

$frases[327][$l] = 'Occupation';

$frases[328][$l] = 'Click on the image or on the name of the course you wish to enter.';

$frases[329][$l] = 'You do not have any assigned loads yet.';

$frases[330][$l] = 'Registration Process';

$frases[331][$l] = 'Data update.';

$frases[332][$l] = 'Payment of the registration.';

$frases[333][$l] = 'Contract.';

$frases[334][$l] = 'Promissory note.';

$frases[335][$l] = 'Academic commitment.';

$frases[336][$l] = 'Coexistence Commitment.';

$frases[337][$l] = 'Coexistence Manual.';

$frases[338][$l] = 'Contract over 14 years old.';

$frases[339][$l] = 'Signed registration.';

$frases[340][$l] = 'The amount payable is';

$frases[341][$l] = 'To make the payment process you have two options, choose one of them';

$frases[342][$l] = 'Enter the link and with the student code pay by pse';

$frases[343][$l] = 'Or pay in the current account of Bancolombia';

$frases[344][$l] = 'Attach support';

$frases[345][$l] = 'Proof of payment';

$frases[346][$l] = 'Note: Once you make the payment, you must attach the support for verification with treasury.';

$frases[347][$l] = 'If you have any problem with the payment please contact the treasury.';

$frases[348][$l] = 'TABLE OF COSTS';

$frases[349][$l] = 'PAYMENT BY PSE';

$frases[350][$l] = 'Send proof of payment';

$frases[351][$l] = 'CONTRACT FOR THE PROVISION OF EDUCATIONAL SERVICES';

$frases[352][$l] = 'In this space you can attach the digital signature, if you do not have a digital signature you can sign on a white paper without lines or squares; take a photo with good lighting and upload it to this media.';

$frases[353][$l] = 'Attach signature.';

$frases[354][$l] = 'NOTE: Once the academic secretary verifies and approves the registration, the final voucher will appear.';

$frases[355][$l] = 'DOWNLOAD VOUCHER';

$frases[356][$l] = 'Main menu';

$frases[357][$l] = 'FIRST STEPS';

$frases[358][$l] = 'First we go to the subjects to select the one we need.';

$frases[359][$l] = 'Lets explore the options of the G. Academic menu';

$frases[360][$l] = 'Lets look at some additional options.';

$frases[361][$l] = 'Video Guide';

$frases[362][$l] = 'All the most important options are in the menu option attended. We recommend you to watch the video on this page so that you have clarity on how to use the platform.';

$frases[367][$l] = 'Cancel service';

$frases[368][$l] = 'Reason for cancellation';

$frases[369][$l] = 'Create';

$frases[370][$l] = 'Available Pages';

$frases[371][$l] = 'Páginas';

$frases[372][$l] = 'Institution';

$frases[373][$l] = 'Year';

$frases[374][$l] = 'Thematic Units';

$frases[375][$l] = 'Edit';

$frases[376][$l] = 'Choose the load and the period from which you want to import the data.';

$frases[377][$l] = 'Now you can specify the data you want to import.';

$frases[378][$l] = 'When importing the grades, the indicators will also be imported automatically.';

$frases[379][$l] = 'Verify that you have selected everything correctly for this import. Once it is done there is no turning back.';

$frases[380][$l] = 'Sales Invoice';

$frases[381][$l] = 'Price';

$frases[382][$l] = 'Tax';

$frases[383][$l] = 'Responsible';

$frases[384][$l] = 'Environment';


$frases[385][$l] = 'Reports';
$frases[386][$l] = 'Search';
$frases[387][$l] = 'More actions';
$frases[388][$l] = 'Questionnaires';
$frases[389][$l] = 'Last login';
$frases[390][$l] = 'Registrations';
$frases[391][$l] = 'Reservation of places';
$frases[392][$l] = 'Registration list';
$frases[393][$l] = 'see questionnaires';
$frases[394][$l] = 'Unlock request';
$frases[395][$l] = 'Of the system';
$frases[396][$l] = 'Of the institution';
$frases[397][$l] = 'Create new Company';
$frases[398][$l] = 'Error log';
$frases[399][$l] = 'Institutions';
$frases[400][$l] = 'Stock history';
$frases[401][$l] = 'Cancellation requests';
$frases[402][$l] = 'Modules';
$frases[403][$l] = 'Pages';
$frases[404][$l] = 'General options';
$frases[405][$l] = 'Restored keys';
$frases[406][$l] = 'Contracts';
$frases[407][$l] = 'Contact information';
$frases[408][$l] = 'Product categories';
$frases[409][$l] = 'Service categories';
$frases[410][$l] = 'Product';
$frases[411][$l] = 'Companies';
$frases[412][$l] = 'Filter by courses';
$frases[413][$l] = 'Payments';
$frases[414][$l] = 'Payment Method';

$frases[415][$l] = 'Recurring Invoices';
$frases[416][$l] = 'Additional notes';
$frases[417][$l] = 'Collected';
$frases[418][$l] = 'To collect';
$frases[419][$l] = 'Save Changes';
$frases[420][$l] = 'Mandatory';
$frases[421][$l] = 'TEXT';
$frases[422][$l] = 'MULTIPLE';
$frases[423][$l] = 'SINGLE';
$frases[424][$l] = 'Client';
$frases[425][$l] = 'Taxe';
$frases[426][$l] = 'Fee';
$frases[427][$l] = 'Graphic Reports';
$frases[428][$l] = 'Additional courses';
