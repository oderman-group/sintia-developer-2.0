...
<?php
$matriculadosPorCurso = mysqli_query($conexion, "SELECT * FROM academico_matriculas WHERE mat_grado='".$_REQUEST["curso"]."' AND mat_eliminado=0 ORDER BY mat_grupo, mat_primer_apellido");
...
$consulta_desempeno=mysqli_query($conexion, "SELECT notip_id, notip_nombre, notip_desde, notip_hasta FROM academico_notas_tipos WHERE notip_categoria=".$config["conf_notas_categoria"].";");	
...
<tr style="font-weight:bold; background:#EAEAEA; border-color:#000; height:20px; color:#000; font-size:10px;">
<td width="20%" align="center">AREAS/ ASIGNATURAS</td>
<td width="2%" align="center">I.H</td>


<?php $columnas=5; for($j=3;$j<=$numero_periodos;$j++){ ?>
...
<td width="5%" align="center">AUS</td>
    ...
    <tr style="background:#F06;">
    	<td class="area" id="" colspan="<?=$columnas;?>" style="font-size:10px; font-weight:bold;"></td>
        <!--<td colspan="3"></td>-->
    </tr>
...
$consulta_a_mat=mysqli_query($conexion, "SELECT (SUM(bol_nota)/".$divP.") as suma,ar_nombre,mat_nombre,mat_id,car_id FROM academico_materias am
INNER JOIN academico_areas a ON a.ar_id=am.mat_area
INNER JOIN academico_cargas ac ON ac.car_materia=am.mat_id
INNER JOIN academico_boletin ab ON ab.bol_carga=ac.car_id
WHERE bol_estudiante='".$matriculadosDatos[0]."' and a.ar_id=".$fila["ar_id"]." and bol_periodo in (".$condicion.")
...
if($total_promedio==1)	$total_promedio="1.0";	if($total_promedio==2)	$total_promedio="2.0";		if($total_promedio==3)	$total_promedio="3.0";	if($total_promedio==4)	$total_promedio="4.0";	if($total_promedio==5)	$total_promedio="5.0";
	if($numfilas_not_area>0){
			?>
  <tr bgcolor="#ABABAB" style="font-size:10px;">
            <td style="font-size:10px; height:25px; font-weight:bold;"><?php echo $resultado_not_area["ar_nombre"];?></td> 
            <td align="center" style="font-weight:bold; font-size:10px;"></td>
            <?php for($k=3;$k<=$numero_periodos;$k++){ 
			?>
...
?>
 <tr bgcolor="#EAEAEA" style="font-size:10px;">
            <td style="font-size:10px; height:35px; font-weight:bold;background:#EAEAEA;">&raquo;<?php echo $fila2["mat_nombre"];?></td> 
            <td align="center" style="font-weight:bold; font-size:10px;background:#EAEAEA;"><?php echo $fila["car_ih"];?></td>
<?php for($l=3;$l<=$numero_periodos;$l++){ 

	$consultaNotaEstudiante=mysqli_query($conexion, "SELECT * FROM academico_boletin WHERE bol_carga='".$fila2['car_id']."' AND bol_estudiante='".$matriculadosDatos[0]."' AND bol_periodo='".$l."'");
	$notaDelEstudiante = mysqli_fetch_array($consultaNotaEstudiante, MYSQLI_BOTH);
?>
			<td class=""  align="center" style="font-weight:bold; background:#EAEAEA; font-size:12px;">
			<?php 
			if($notaDelEstudiante['bol_nota']!=""){
				$consultaDesempenoNotaP=mysqli_query($conexion, "SELECT * FROM academico_notas_tipos WHERE notip_categoria='".$config[22]."' AND ".$notaDelEstudiante['bol_nota'].">=notip_desde AND ".$notaDelEstudiante['bol_nota']."<=notip_hasta");
...
<tr bgcolor="#FFF" style="font-size:10px;">
            <td style="font-size:10px; height:15px;"><?php echo $contador_indicadores.". ".$fila4["ind_nombre"];?></td> 
            <td align="center" style="font-weight:bold; font-size:10px;"></td>
            <?php for($m=3;$m<=$numero_periodos;$m++){ ?>
...
<?php }}//while fin areas?>
    ...
    <tr align="center" style="font-size:10px; font-weight:bold;">
		...
		<?php for($n=3;$n<=$numero_periodos;$n++){ ?>
...
<?php for($n=3;$n<=$numero_periodos;$n++){if($promedios[$n]!=0){$promedios[$n]=0; $contpromedios[$n]=0;} } ?>
...
<table width="100%" id="tblBoletin" cellspacing="0" cellpadding="0" rules="all" border="1" align="center">
    ...
    <tr style="font-weight:bold; background:#036; border-color:#036; height:40px; color:#FC0; font-size:10px; text-align:center">
    	<td colspan="3">NOTA DE COMPORTAMIENTO</td>
    </tr>
    
    <tr style="font-weight:bold; background:#F06; border-color:#F06; height:25px; color:#FFF; font-size:10px; text-align:center">
...
?>
    <tr align="center" style="font-weight:bold; font-size:10px; height:20px;">
...
<div align="center">
<table width="100%" cellspacing="0" cellpadding="0"  border="0" style="text-align:center; font-size:10px;">
  <tr>
