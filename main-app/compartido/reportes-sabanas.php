...
$asig=mysqli_query($conexion, "SELECT * FROM $BD.academico_matriculas WHERE mat_grado='".$_REQUEST["curso"]."' AND mat_grupo='".$_REQUEST["grupo"]."' AND (mat_estado_matricula=1 OR mat_estado_matricula=2) AND mat_eliminado=0 ORDER BY mat_primer_apellido");			
$num_asg=mysqli_num_rows($asig);
$consultaGrados=mysqli_query($conexion, "SELECT * FROM $BD.academico_grados, academico_grupos WHERE gra_id='".$_REQUEST["curso"]."' AND gru_id='".$_REQUEST["grupo"]."'");
...
<div align="center" style="margin-bottom:20px;">
   <img src="../files/images/logo/<?=$informacion_inst["info_logo"]?>" height="150" width="250"><br>
    ...
    INFORME DE SABANAS - PERIODO: <?=$_REQUEST["per"];?></br>
    <b><?=strtoupper($grados["gra_nombre"]." ".$grados["gru_nombre"]." ".$year);?></b><br>
</div>   
  ...
  <tr style="font-weight:bold; font-size:12px; height:30px; background:<?php echo $config[12] ?>;">
        ...
        <?php
		$materias1=mysqli_query($conexion, "SELECT * FROM $BD.academico_cargas WHERE car_curso=".$_REQUEST["curso"]." AND car_grupo='".$_REQUEST["grupo"]."'");
		while($mat1=mysqli_fetch_array($materias1, MYSQLI_BOTH)){
			$nombresMat=mysqli_query($conexion, "SELECT * FROM $BD.academico_materias WHERE mat_id=".$mat1[4]);
  ...
  {
  		$cuentaest=mysqli_query($conexion, "SELECT * FROM $BD.academico_boletin WHERE bol_estudiante=".$fila[0]." AND bol_periodo=".$_REQUEST["per"]." GROUP BY bol_carga");
  ...
  <tr style="font-size:13px;">
      ...
      <!--<td align="center"><?php if($fila[7]==1)echo "A"; else echo "B";?></td> -->
       <?php
		...
		$materias1=mysqli_query($conexion, "SELECT * FROM $BD.academico_cargas WHERE car_curso=".$_REQUEST["curso"]." AND car_grupo='".$_REQUEST["grupo"]."'");
		while($mat1=mysqli_fetch_array($materias1, MYSQLI_BOTH)){
			$notas=mysqli_query($conexion, "SELECT * FROM $BD.academico_boletin WHERE bol_estudiante=".$fila[0]." AND bol_carga=".$mat1[0]." AND bol_periodo=".$_REQUEST["per"]);
