...
$cursoActual=GradoServicios::consultarCurso($_GET["curso"]);
...
<div style="margin: 10px;">
  ...
  <tr style="font-weight:bold; font-size:12px; height:30px; background:#6017dc; color:white;">
        ...
        <?php
		$materias1=mysqli_query($conexion, "SELECT * FROM academico_cargas WHERE car_curso=".$_GET["curso"]." AND car_grupo='".$_GET["grupo"]."' AND car_activa=1");
  ...
  <tr style="font-size:13px;">
      ...
      <!--<td align="center"><?php if($fila[7]==1)echo "A"; else echo "B";?></td> -->
       <?php
		...
		$def='0.0';
		while($mat1=mysqli_fetch_array($materias1, MYSQLI_BOTH)){
			$notas=mysqli_query($conexion, "SELECT * FROM academico_boletin WHERE bol_estudiante=".$fila['mat_id']." AND bol_carga=".$mat1[0]." AND bol_periodo=".$_GET["per"]);
