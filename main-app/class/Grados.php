...
class Grados {
    ...
    **/
    ...
    /**
     * Me trae la configuracion para el boletin de una institucion
     *
     * @param array $config
     * 
     * @return array $resultado
     *
     */
    public static function traerConfiguracionBoletin(
        array $config
    ){

        $sql = "SELECT * FROM ".BD_ADMIN.".configuracion_boletin WHERE conbol_institucion=? AND conbol_year=?";
    ...
    **/
    public static function guardarConfiguracionBoletin (
        string  $insert,
        array   $parametros
    )
    {
        ...
        $sql = "INSERT INTO ".BD_ADMIN.".configuracion_boletin({$insert}) VALUES ({$signosPreguntas})";
        
        $resultado = BindSQL::prepararSQL($sql, $parametros);
    }
    /**
     * Me actualiza la configuracion para el boletin de una institucion
     *
     * @param int       $idConf
     * @param string    $update
     */
    public static function actualizarConfiguracionBoletin(
        int     $idConf,
        string  $update
    )
    {
        ...
        $sql = "UPDATE ".BD_ADMIN.".configuracion_boletin SET {$updateSql} WHERE conbol_id=? AND conbol_institucion=? AND conbol_year=?";

        $parametros = array_merge($updateValues, [$idConf, $_SESSION["idInstitucion"], $_SESSION["bd"]]);
