...
<?php
	...
	$consultaUsuario2 = mysqli_query($conexionBaseDatosServicios, "
	SELECT uss_usuario FROM ".BD_GENERAL.".usuarios 
	WHERE 
	(
		uss_usuario='".trim($_POST["Usuario"])."' OR 
		uss_usuario LIKE '".trim($_REQUEST["Usuario"])."%' 
	) 
	AND TRIM(uss_usuario)!='' 
	AND uss_clave=SHA1('".$_POST["Clave"]."')  
	AND uss_usuario IS NOT NULL
	");

	$datosUsuario2 = mysqli_fetch_array($consultaUsuario2, MYSQLI_BOTH);

	$suma       = isset($_REQUEST["suma"]) ? base64_encode($_REQUEST["suma"]) : null;
	$sumaReal   = isset($_REQUEST["sumaReal"]) ? base64_encode($_REQUEST["sumaReal"]) : null;
	$urlDefault = isset($_REQUEST["urlDefault"]) ? base64_encode($_REQUEST["urlDefault"]) : null;
	$directory  = isset($_REQUEST["directory"]) ? base64_encode($_REQUEST["directory"]) : null;

	header("Location:autentico.php?Usuario=".base64_encode($datosUsuario2["uss_usuario"])."&Clave=".base64_encode($_REQUEST["Clave"])."&suma=".$suma."&sumaReal=".$sumaReal."&urlDefault=".$urlDefault."&directory=".$directory);
	exit();
}