...
while ($est = mysqli_fetch_array($estud, MYSQLI_BOTH)) {
	...
	if ( empty($usComp) ) {
		try{
			mysqli_query($conexion, "DELETE FROM ".BD_GENERAL.".usuarios WHERE uss_usuario='" . $est[12] . "' AND institucion={$config['conf_id_institucion']} AND year={$_SESSION["bd"]}");
		} catch (Exception $e) {
