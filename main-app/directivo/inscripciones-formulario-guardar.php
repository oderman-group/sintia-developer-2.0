...
if (!empty($_FILES['foto']['name'])) {
	$destino = ROOT_PATH.'/main-app/admisiones/files/fotos';
...
header('Location:inscripciones-formulario.php?msg='.base64_encode(3).'&token=' . md5($_POST['solicitud']) . '&idInst=' . $_REQUEST['idInst'] . '&id=' . base64_encode($_POST['solicitud']) . '&fa=' . base64_encode($filasAfectadas) . '&faa=' . base64_encode($filasAfectadasAcu));
exit();
