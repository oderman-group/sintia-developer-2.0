...
while ($contCurso < $numCurso) {
    ...
    while ($contGrupo < $numGrupo) {
        ...
        while ($contAsignatura < $numAsignatura) {
            ...
            if(!$existeCarga) {
                $idInsercion = CargaAcademica::guardarCarga($conexionPDO, "car_docente, car_curso, car_grupo, car_materia, car_periodo, car_activa, car_permiso1, car_director_grupo, car_ih, car_fecha_creada, car_responsable, car_maximos_indicadores, car_maximas_calificaciones, car_configuracion, car_valor_indicador, car_indicador_automatico, car_observaciones_boletin, car_posicion_docente, institucion, year, car_id", [$_POST["docente"], $_POST["curso"][$contCurso], $_POST["grupo"][$contGrupo],$_POST["asignatura"][$contAsignatura], $_POST["periodo"], 1, 1, $_POST["dg"], $_POST["ih"], date("Y-m-d H:i:s"), $_SESSION["id"], $_POST["maxIndicadores"], $_POST["maxActividades"], $_POST["valorActividades"], $_POST["valorIndicadores"], $_POST["indicadorAutomatico"], 0, 1, $config['conf_id_institucion'], $_SESSION["bd"]]);
                $cargasCreadas ++;
            } else {
                $cargasNoCreadas ++;
            }
