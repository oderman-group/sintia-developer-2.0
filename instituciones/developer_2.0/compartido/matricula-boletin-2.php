...
$consulta_a_mat_per=mysql_query("SELECT bol_nota,bol_periodo,ar_nombre,mat_nombre,mat_id, car_id FROM academico_materias am
INNER JOIN academico_areas a ON a.ar_id=am.mat_area
INNER JOIN academico_cargas ac ON ac.car_materia=am.mat_id
INNER JOIN academico_boletin ab ON ab.bol_carga=ac.car_id
WHERE bol_estudiante='".$_GET["id"]."' and a.ar_id=".$fila["ar_id"]." and bol_periodo in (".$condicion.")
...
WHERE car_curso=".$datos_usr["mat_grado"]."  and car_grupo=".$datos_usr["mat_grupo"]." and mat_area=".$fila["ar_id"]." AND ipc_periodo in (".$condicion.") AND cal_id_estudiante='".$_GET["id"]."' and act_periodo=".$condicion2."
...
for($l=1;$l<=$numero_periodos;$l++){ 
	//$numNotaDelEstudiante = mysql_num_rows(mysql_query("SELECT * FROM academico_boletin WHERE bol_carga='".$fila2['car_id']."' AND bol_estudiante='".$_GET["id"]."' AND bol_periodo='".$l."'",$conexion));
	$notaDelEstudiante = mysql_fetch_array(mysql_query("SELECT * FROM academico_boletin WHERE bol_carga='".$fila2['car_id']."' AND bol_estudiante='".$_GET["id"]."' AND bol_periodo='".$l."'",$conexion));
?>
	  ...
	  $total_promedio2=round( $fila2["suma"],1);
	   ...
	   if($total_promedio2<$config[5]){
		   $nivelaciones = mysql_fetch_array(mysql_query("SELECT * FROM  academico_nivelaciones WHERE niv_id_asg='".$fila2['car_id']."' AND niv_cod_estudiante='".$_GET["id"]."'",$conexion));
...
<p>&nbsp;</p>
<?php 
$cndisiplina = mysql_query("SELECT * FROM disiplina_nota WHERE dn_cod_estudiante='".$_GET["id"]."' AND dn_periodo in(".$condicion.");",$conexion);
